...
export class AuthController {
  ...
  @ApiResponse({ status: 400, description: 'Invalid school code' })
  ...
  @Get('public/schools/:code/academic-years')
  @Public()
  @ApiOperation({ summary: 'Get academic years for a school (for student registration)' })
  @ApiParam({ name: 'code', description: 'School code' })
  @ApiResponse({ status: 200, description: 'List of academic years' })
  async getPublicSchoolAcademicYears(@Param('code') code: string) {
    return this.authService.getPublicSchoolAcademicYears(code);
  }
  ...
  @UseGuards(JwtAuthGuard, PermissionsGuard)
  @RequirePermissions('teacher:approve')
  @ApiBearerAuth('JWT-auth')
